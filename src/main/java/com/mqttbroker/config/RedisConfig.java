...
public class RedisConfig {
    ...
    @Value("${cache.off-memory.redis.cluster.max-redirects}")
    private int maxRedirects;

    @Bean(name = "offMemoryCache")
    ...
    @Bean
    @ConditionalOnProperty(name = "cache.off-memory.enabled", havingValue = "true")
    public RedisTemplate<String, Object> redisTemplate(@Qualifier("offMemoryCache") LettuceConnectionFactory connectionFactory) {
        ...
        log.info("RedisTemplate instance created redisTemplate={}", template);
        return template;
    }
